...
document.addEventListener('DOMContentLoaded', function () {
    ...
    filterButtons.forEach(filterButton => {
        filterButton.addEventListener('click', () => {
            // if all, return all menu cards
            if (filterButton.value == "All") return createMenuCards(menuArray);

            // go to filtering and get the values
            const menuArrayFiltered = menuArray.filter(menu => {
                return menu.categoryName == filterButton.value;
            });

            // create a menu card for filtered menu
            createMenuCards(menuArrayFiltered);
        });
    });
